...
/*   Updated: 2026/06/24 15:31:05 by cybourge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	test_scn4(t_scn *scene)
{
	...
	const t_pt	from = pt_crt(0, 20, -40);
	...
	// trf_ini(&transforms);
	// trf_scl(&transforms, 0.1, 0.1, 0.1);
	// trf_rot(&transforms, 0, 0, PI/2);
	// trf_trf(&transforms);
	...
	// trf_ini(&transforms);
	// trf_scl(&transforms, 5, 5, 5);
	// trf_rot(&transforms, 0, PI/4.0, 0);
	// trf_trf(&transforms);
	// pat_trf(&(plane->mtrl.pat), &transforms);
	// plane->mtrl.pat.pat = pat_chkr1;
	// plane->mtrl.pat.clr1 = clr_unpack(WHITE);
	// plane->mtrl.pat.clr2 = clr_unpack(RED);
	// index++;
